...
async function main() {
  ...
  const vectorizedDoc: number[][] = await embedAndVectorizedContents(
    LLM_MODEL_EMBEDDED,
    documents,
  );
  ...
  const query = "経費の締め日を教えて";
  console.log(`クエリ: "${query}"\n`);
  const vectorizedQuery: number[] = await embedAndVectorizedContent(
    LLM_MODEL_EMBEDDED,
    query,
  );

  // 類似度計算 & 最も関連度の高いドキュメントの特定 (Top 1)
 ...
 * @returns 埋め込みベクトル
 */
async function embedAndVectorizedContent(
  model: string,
  content: string,
): Promise<number[]> {
 ...
 * @returns 埋め込みベクトル
 */
async function embedAndVectorizedContents(
  model: string,
  contents: string[],
): Promise<number[][]> {
